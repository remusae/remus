...
def crypto_aead_encrypt(M, A, N, K):
    ...
    m = len(M_parsed)-1
    if len(M_parsed[m]) < B_LENGTH: wm = 15 
    else: wm = 14
    z = len(M_parsed[m])
    ...
    for i in range(1,m+1):
        S, _ = rho(S, M_parsed[i])
        L = increase_counter(L)
        V = increase_counter(V)
        tweak = L[:]
        if i<m: tweak[15] = tweak[15] ^ MEMBER_MASK ^ 6
    ...
    for i in range(1,m+1):
        tweak = L[:]
        tweak[15] = tweak[15] ^ MEMBER_MASK ^ 2
        S = [S[j] ^ V[j] for j in range(B_LENGTH)]
        S = skinny_enc(S, tweak, SKINNY_VERSION)
        S = [S[j] ^ V[j] for j in range(B_LENGTH)]
...
def crypto_aead_decrypt(C, A, N, K):
    ...
    c = len(C_parsed)-1
    if len(C_parsed[c]) < B_LENGTH: wc = 15 
    else: wc = 14
    z = len(C_parsed[c])
